...
void Image::getMask() {
	...
	while (1) {
		...
		if ((mouse_event == EVENT_MOUSEMOVE && (mouse_flags == EVENT_FLAG_LBUTTON)) || (mouse_event == EVENT_LBUTTONDOWN)) {
			if (first) {
				last_pos = mouse_pos;
				first = false;
			}
...
void Image::getCurves() {
	...
	while (true) {
		...
		else if ((mouse_event == EVENT_MOUSEMOVE && (mouse_flags == EVENT_FLAG_LBUTTON)) || (mouse_event == EVENT_LBUTTONDOWN)) {
			if (first == true) {
				first = false;
				last_pos = mouse_pos;
				continue;
